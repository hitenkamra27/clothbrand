...
const VALID_PAYMENT_METHODS = ['cod', 'shiprocket'];
...
function validateOrder(body) {
  ...
  if (!VALID_PAYMENT_METHODS.includes(paymentMethod)) {
    errors.push('paymentMethod must be cod or shiprocket');
  }
...
router.post('/', orderLimiter, async (req, res) => {
  ...
  const order = {
    id,
    customerId: (req.session && req.session.customerId) || null,
    items: cleanItems,
    subtotal,
    customer: {
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  let shiprocketWarning = null;
  // Moving an order to "processing" is the trigger to actually book it with
  // Shiprocket, so the courier pickup/tracking flow starts. Only do this
  // once per order, and only if Shiprocket credentials are set — otherwise
  // the site keeps working exactly as before (manual fulfillment).
  if (status === 'processing' && !order.shiprocket && shiprocket.isConfigured()) {
    try {
      ...
      shiprocketWarning = err.message;
    }
  }

  await db.write('orders', orders);
  res.json(shiprocketWarning ? { ...order, shiprocketWarning } : order);
});
