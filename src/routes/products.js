...
function validateProduct(body, { partial = false } = {}) {
  ...
  if (body.description !== undefined) {
    ...
    else p.description = body.description.trim();
  }

  if (body.stock !== undefined) {
    if (body.stock === null || body.stock === '') {
      p.stock = null; // null = unlimited / not tracked
    } else {
      const stock = Number(body.stock);
      if (!Number.isFinite(stock) || stock < 0) errors.push('stock must be a positive number or empty');
      else p.stock = Math.floor(stock);
    }
  }
  ...
  if (body.tags !== undefined) {
    if (Array.isArray(body.tags)) {
      ...
      errors.push('tags must be a comma-separated string or an array');
    }
  }
  ...
  if (body.images !== undefined) {
    if (!Array.isArray(body.images) || !body.images.every(s => typeof s === 'string')) {
      ...
      p.images = body.images.map(s => s.trim()).filter(Boolean);
    }
  }
  ...
  if (p.gender && !['men', 'women', 'unisex'].includes(p.gender)) {
