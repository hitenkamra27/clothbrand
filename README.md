...
- **Products** — add, edit, delete. Pick a gender, category, and type; check
  ...
  Sellers, or any custom section you've added).
...
Product images are given as URLs (paste a link to an image you've uploaded
somewhere, e.g. your own hosting, Cloudinary, or similar) — there's no file
upload yet. That's the next thing worth adding if you want it.
...
GoKwik), product detail pages (currently Quick View covers this on category
...
Products/categories/sections are stored as JSON files on disk
(`/data/*.json`). This is simple and fine for a single-admin boutique
catalog. Two things to know:
- Some hosts (like Render's free tier) use an ephemeral filesystem — your
  data can reset on redeploy/restart unless you attach a persistent disk
  (Render has a "Disks" add-on for this) or migrate to a real database.
- If you outgrow this (multiple admins, thousands of products, image
  uploads), the next step is swapping `src/db.js` for a proper database
  (Postgres is a common, well-supported choice on both Render and Railway).
  The route files (`src/routes/*.js`) wouldn't need to change much since
  they're already isolated from the storage layer.

## Project structure

```
server.js                 Express app, sessions, security headers, routing
src/db.js                 JSON file read/write layer (used by all routes)
...
data/*.json               Your actual product/category/section data
...
admin/dashboard.html      Admin dashboard (Products / Categories / Sections)
