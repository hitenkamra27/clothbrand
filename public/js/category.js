(function(){
  ...
  const HERO_MAP = {
    ...
    kids: { title: 'Kids', eyebrow: 'For Them', desc: 'Durable, comfortable staples built for play — sized right, priced fair.', img: 'https://picsum.photos/seed/herokids/1800/900' },
    ...
    search: { title: 'Search Results', eyebrow: 'STUDIO', desc: '', img: 'https://picsum.photos/seed/herosearch/1800/900' },
  ...
  function currentHeroKey(state){
    ...
    if(state.gender === 'kids') return 'kids';
    return 'all';
  }
  ...
  function renderHero(state){
    ...
    document.getElementById('pageTitle').textContent = `${hero.title} — STUDIO`;
  }
  ...
  function renderPagination(){
    if(totalPages <= 1){ paginationEl.innerHTML = ''; loadMoreBtn.hidden = true; return; }

    let html = `<button class="cat-page-btn" id="pagePrev" ${currentPage <= 1 ? 'disabled' : ''}>‹</button>`;
    for(let i = 1; i <= totalPages; i++){
      if(i === 1 || i === totalPages || Math.abs(i - currentPage) <= 1){
        html += `<button class="cat-page-btn${i === currentPage ? ' active' : ''}" data-page="${i}">${i}</button>`;
      } else if(Math.abs(i - currentPage) === 2){
        html += `<span style="padding:0 4px;color:var(--grey-light);">…</span>`;
      }
    }
    html += `<button class="cat-page-btn" id="pageNext" ${currentPage >= totalPages ? 'disabled' : ''}>›</button>`;
  ...
  paginationEl.addEventListener('click', (e) => {
    const btn = e.target.closest('button[data-page], #pagePrev, #pageNext');
    if(!btn) return;
    let page = currentPage;
    if(btn.id === 'pagePrev') page = Math.max(1, currentPage - 1);
    else if(btn.id === 'pageNext') page = Math.min(totalPages, currentPage + 1);
    else page = parseInt(btn.dataset.page, 10);
    const state = getState();
    const usp = new URLSearchParams(window.location.search);
    usp.set('page', page);
    window.history.pushState({}, '', '/category.html?' + usp.toString());
    loadAndRender({ append: false });
  ...
  function setupInfiniteScroll(){
    ...
    observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if(entry.isIntersecting && !loading && currentPage < totalPages && !loadMoreBtn.hidden){
  ...
  async function loadAndRender({ append }){
    ...
    await loadPage(state.page || 1, { append });
    setupInfiniteScroll();
  }
  ...
  function openQuickView(product){
    qvProduct = product;
    document.getElementById('qvImg').src = product.imgFront;
