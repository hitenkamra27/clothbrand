(function(){
  ...
  function renderLayout(p){
    document.getElementById('pageTitle').textContent = `${p.name} — Hive & Ash`;
    const sizes = String(p.sizes || '').split('·').map(s => s.trim()).filter(Boolean);
    const onSale = p.salePrice != null && p.salePrice < p.price;
    const images = [p.imgFront, p.imgBack, ...(Array.isArray(p.images) ? p.images : [])]
      .filter(Boolean)
    ...
    layout.innerHTML = `
      <div class="pdp-gallery">
        <div class="pdp-gallery-main" id="pdpGalleryMain">
          <div class="pdp-gallery-track" id="pdpGalleryTrack" style="width:${images.length * 100}%;">
            ${images.map(img => `
              <div class="pdp-gallery-slide" style="width:${100 / images.length}%;">
                <img src="${escapeHtml(img)}" alt="${escapeHtml(p.name)}">
              </div>`).join('')}
          </div>
          ${images.length > 1 ? `
            <button type="button" class="pdp-gallery-arrow prev" id="pdpArrowPrev" aria-label="Previous image">
              <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
            </button>
            <button type="button" class="pdp-gallery-arrow next" id="pdpArrowNext" aria-label="Next image">
              <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg>
            </button>
            <div class="pdp-gallery-dots" id="pdpGalleryDots">
              ${images.map((_, i) => `<span class="${i === 0 ? 'active' : ''}" data-dot="${i}"></span>`).join('')}
            </div>` : ''}
        </div>
        ${images.length > 1 ? `
          <div class="pdp-gallery-thumbs" id="pdpThumbs">
            ${images.map((img, i) => `<button class="${i === 0 ? 'active' : ''}" data-index="${i}"><img src="${escapeHtml(img)}" alt=""></button>`).join('')}
          </div>` : ''}
      </div>
      <div class="pdp-info">
        <div class="pdp-brand">${escapeHtml(p.brand || 'Hive & Ash')}</div>
        ...
        <div class="pdp-accordion">
          <div class="pdp-accordion-item open">
            ...
            <div class="pdp-accordion-body">${escapeHtml(p.name)} in ${escapeHtml(p.color || 'a considered colorway')}, made from ${escapeHtml(p.material || 'quality materials')}. Part of the ${escapeHtml(p.brand || 'Hive & Ash')} line.</div>
          </div>
          <div class="pdp-accordion-item">
            ...
            <div class="pdp-accordion-body" id="pdpShippingCopy">Free shipping on orders over ₹${freeShipThreshold().toLocaleString('en-IN')}. Estimated delivery in 3–6 business days. Easy 14-day returns on unworn items with tags attached.</div>
          </div>
          <div class="pdp-accordion-item">
    ...
    // Gallery: sliding track (arrows, thumbnail clicks, dots, and touch swipe)
    ...
    const prevArrow = document.getElementById('pdpArrowPrev');
    const nextArrow = document.getElementById('pdpArrowNext');
    const slideCount = images.length;
    let slideIndex = 0;

    function goToSlide(i){
      slideIndex = ((i % slideCount) + slideCount) % slideCount;
      if(track) track.style.transform = `translateX(-${slideIndex * (100 / slideCount)}%)`;
      if(thumbs) thumbs.querySelectorAll('button').forEach((b, idx) => b.classList.toggle('active', idx === slideIndex));
      if(dots) dots.querySelectorAll('span').forEach((d, idx) => d.classList.toggle('active', idx === slideIndex));
    }
    ...
    if(track && slideCount > 1){
      ...
      if(prevArrow) prevArrow.addEventListener('click', () => goToSlide(slideIndex - 1));
      if(nextArrow) nextArrow.addEventListener('click', () => goToSlide(slideIndex + 1));
      ...
      galleryMain.addEventListener('touchend', () => {
        ...
        if(Math.abs(touchDeltaX) > 40){
          goToSlide(touchDeltaX < 0 ? slideIndex + 1 : slideIndex - 1);
        }
      });
    }
  ...
  function wireChrome(){
    ...
    // backed by /api/settings — re-render/update once it's loaded for real.
    if (typeof getSiteSettings === 'function') {
      getSiteSettings().then(s => {
        if (!s) return;
        renderCart();
        const copy = document.getElementById('pdpShippingCopy');
        if (copy) copy.textContent = `Free shipping on orders over ₹${freeShipThreshold().toLocaleString('en-IN')}. Estimated delivery in 3–6 business days. Easy 14-day returns on unworn items with tags attached.`;
      });
    }
