(function () {
  ...
  if (typeof getSiteSettings === 'function') {
    getSiteSettings().then(s => {
      if (!s) return;
      renderCart();
      const fee = document.getElementById('infoShippingFee');
      if (fee) fee.textContent = `₹99 (Free over ₹${freeShipThreshold().toLocaleString('en-IN')})`;
      const emailEl = document.getElementById('infoContactEmail');
      if (emailEl && s.contactEmail) emailEl.textContent = s.contactEmail;
      const phoneEl = document.getElementById('infoContactPhone');
      if (phoneEl && s.contactPhone) phoneEl.textContent = `${s.contactPhone} (Mon–Sat, 10am–6pm IST)`;
    });
  }
  ...
  const PAGES = {
    about: {
      ...
      body: `
        ...
        <p>Hive & Ash began as a two-person operation working out of a shared studio space, frustrated with how much clothing was designed to be replaced rather than repaired. We set out to build a wardrobe of fewer, better pieces — garments you'd actually want to keep wearing five years on.</p>
        <p style="margin-top:10px;">Hive & Ash is run by Kamra & Sons Pvt Ltd, based in Hisar, Haryana.</p>
    ...
    'shipping-returns': {
      ...
      body: `
        ...
        <table class="info-table">
          ...
          <tbody>
            <tr><td>Standard</td><td>3–6 business days</td><td id="infoShippingFee">₹99 (Free over ₹1,999)</td></tr>
    ...
    return `
      <div class="info-contact-grid">
        <div class="info-contact-details">
          ...
          <p id="infoContactEmail">${(window.__siteSettings && window.__siteSettings.contactEmail) || 'hiveandash@gmail.com'}</p>
          <div class="field-label">Phone</div>
          <p id="infoContactPhone">${(window.__siteSettings && window.__siteSettings.contactPhone) ? `${window.__siteSettings.contactPhone} (Mon–Sat, 10am–6pm IST)` : '+91 70159 41808 (Mon–Sat, 10am–6pm IST)'}</p>
          <div class="field-label">Address</div>
          <p>Kamra & Sons Pvt Ltd, Hisar, Haryana 125001</p>
        </div>
  ...
  function render() {
    ...
    document.title = `${page.title} — Hive & Ash`;
    document.getElementById('pageTitle').textContent = `${page.title} — Hive & Ash`;
